...
const fetchReligions = async (pageNumber = 1) => {
  setListLoading(true);
  
  try {
    // Ensure no double slash
    const url = `${API_URL.replace(/\/$/, "")}/api/admin/religion?page=${pageNumber}&limit=${PAGE_SIZE}`;
    ...
    if (response.ok) {
      ...
      setTotalPages(result.totalPages || 1); // assuming result.total has total items
        ...
        console.error(result.message || "Failed to fetch religions.");
      }
    } catch (err) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const url = editId
        ? `${API_URL.replace(/\/$/, "")}/api/admin/religion/update/${editId}`
        : `${API_URL.replace(/\/$/, "")}/api/admin/religion/create`;
      ...
      if (response.ok) {
         setSuccess(editId ? "Religion updated successfully!" : "Religion added successfully!");
        reset(); // clear form
        setEditId(null);
        fetchReligions(page);
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API_URL.replace(/\/$/, "")}/api/admin/religion/delete/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const result = await response.json();
      if (response.ok) {
        setSuccess("Religion deleted successfully!");
        fetchReligions(page);
      } else {
                        ...
                        <div className="col-12 col-sm-4 pr-lg-0 mb-3">
                          <label>Enter Religion</label>
                           <input
                            ...
                            {...register("name", { required: "Please enter religion name" })}
                          />
                             {errors.religion && (
