...
export default function WorkingWith() {
  ...
  const fetchWorkingwith = async (pageNumber = 1) => {
    setListLoading(true);

    try {
      // Ensure no double slash
      const url = `${API_URL.replace(/\/$/, "")}/api/admin/working-with?page=${pageNumber}&limit=${PAGE_SIZE}`;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const url = editId
                  ? `${API_URL.replace(/\/$/, "")}/api/admin/working-with/update/${editId}`
                  : `${API_URL.replace(/\/$/, "")}/api/admin/working-with/create`;
      ...
      if (response.ok) {
        setSuccess("Working with added successfully.");
        reset(); // clear form
        fetchWorkingwith(page);
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API_URL.replace(/\/$/, "")}/api/admin/working-with/delete/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const result = await response.json();
      if (response.ok) {
        setSuccess("Working with deleted successfully.");
        fetchWorkingwith(page);
      } else {
        setSuccess(result.message || "Failed to delete working with.");
      }
    } catch (err) {
      console.error(err);
                    ...
                    <div className="filtercont">
                      <div className="row">
                        <div className="col-12 col-sm-4 pr-lg-0 mb-3">
                          <label>Enter Working With</label>
                          <input
                            type="text"
                            className={`form-control ${errors.name ? "is-invalid" : ""}`}
                            {...register("name", { required: "Please enter working with" })}
                          />
                          {errors.name && (
