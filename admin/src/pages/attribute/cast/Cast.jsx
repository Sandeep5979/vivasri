...
export default function Cast() {
  ...
  const fetchReligion = useCallback(async () => {
  try {
   ...
   const fetchCaste = useCallback(
    async (pageNumber = 1) => {
      setListLoading(true);
      try {
        const url = `${API_URL.replace(/\/$/, "")}/api/admin/caste?page=${pageNumber}&limit=${PAGE_SIZE}`;
        const response = await fetch(url, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const result = await response.json();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const url = editId
        ? `${API_URL.replace(/\/$/, "")}/api/admin/caste/update/${editId}`
        : `${API_URL.replace(/\/$/, "")}/api/admin/caste/create`;
      ...
      if (response.ok) {
        setSuccess("Caste added successfully!");
        reset(); // clear form
        setEditId(null);
        fetchCaste(page);
      } else {
        setSuccess(result.message || "Failed to add religion.");
      }
    } catch (err) {
  ...
  const handleEdit = (caste) => {
    setValue("religion_id", caste.religion_id._id);
    setValue("name", caste.name);
    setValue("isActive", caste.status === "Active");
    setEditId(caste._id); // store id for update
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API_URL.replace(/\/$/, "")}/api/admin/caste/delete/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      const result = await response.json();
      if (response.ok) {
        setSuccess("Caste deleted successfully!");
        fetchCaste(page);
      } else {
                      ...
                      <div className="row">
                        ...
                        <div className="col-12 col-sm-4 pr-lg-0 mb-3">
                          ...
                          <input
                            ...
                            {...register("name", {
                              required: "Please enter cast name",
